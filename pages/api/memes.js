...
export default async function handler(req, res) {
  ...
  switch (req.method) {
    case "GET":
      ...
      const { sort = 'new', categories = 'all', tags = 'all', edge = 0, from = 0 } = req.query;
      ...
      const pipeline = [
        ...
        },
        {
          $addFields: {
            ...
            topCategories: {$map: {input: {$filter: {
        ...
        },
        {
          ...
          $unset: ['downloader', '_id', 'votes', 'edgevotes', 'descriptions', 'transcriptions', 'tags', 'categories', 'discordOrigin', 'originalUrl', 'url']
        },
        {
          $match: {
