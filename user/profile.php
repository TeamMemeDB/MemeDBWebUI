...
function getuser($id): User{
    ...
    $result = $conn->query("SELECT Username,Discriminator,Avatar,Admin,Banned,FavouritesPrivacy FROM user WHERE Id = ".$id);
    if($result){
			$row = $result->fetch_assoc();
...
function superprofile(User $user){
	...
	if(strlen($badges) == 0) $badges = "<span class=\"badge\" title=\"This user has no badges yet.\">No badges</span>";
	
	echo "
	<div class=\"super\">
		<div class=\"super-content\">
			<h2>$user->username<span class=\"dim\">$user->discriminator</span></h2>
			<p>$badges</p>
		</div>
		<div class=\"super-footer\">
			<p>
				".($user->isme?"<a href=\"/login/?logout\" class=\"btn danger-bg light\">Logout</a>":($user->id != '309270899909984267'?"<a href=\"/report/?user=$user->id\" class=\"btn danger-bg light\">Report</a>":""))."
		...
		<div class=\"super-bg\">
			<img class=\"float-right\" id=\"userphppfp\" alt=\"$user->username's profile picture\" src=\"https://cdn.discordapp.com/avatars/$user->id/$user->avatar.jpg?size=256\">
		</div>
	</div>";
}
?>